...
async def async_timer(timer_sec, story):
	await asyncio.sleep(timer_sec)
	story.cancel()

async def async_story(node_id, nodes, state):
	try:
		while True:
			clear_screen()
			if (node_id == "room_0"):
...
def time_to_eat(nodes, last_node):
	...
	if node:
		...
		print(f"ノード'{end_node_id}'が見つかりません。")
...
async def main():
	...
	story = asyncio.create_task(async_story(node_id, nodes, state))
	await_timer = asyncio.create_task(async_timer(timer_sec, story))

	await await_timer
	last_node = await story

	time_to_eat(nodes, last_node)
